...
>
> **이 문서는 계획으로 쓰였고, 아래 §7이 실행 기록이다.**
>
> **감사가 든 결함은 전부 처리됐다.** 1~5단계와, 6단계에 있던 결함성 항목 넷
> (EDNS Cookie, BADVERS, L4, I1)까지. L4는 고친 것이 아니라 **틀린 발견이어서
> 철회**했고, 그 판단으로 배포됐던 거짓 경고도 되돌렸다.
>
> 6단계에 남은 셋(L5·L6·존별 RBAC)은 결함이 아니라 리포트 §9의 **기능 제안**이다.
> 요청 없이 만드는 것은 범위를 넓히는 일이라 하지 않았다 — §7이 그 구분을 든다.
>
> 완료는 이 문서가 아니라 커밋과 테스트가 증명하며, §7이 항목마다 커밋 sha를 든다.
>
...
1~5단계 완료, 6단계는 결함성 항목 넷까지, 그리고 §9 제안 중 둘. 기준선
`95e85e6`(테스트 667) → `pnpm test` **713/713**, `pnpm check`·`pnpm build` 통과,
`verify:dns`·`verify:proxy`·`verify:postgres` 통과.

| 단계 | 항목 | 커밋 | 결과 |
| --- | --- | --- | --- |
| 1 | H1 | `ad0247e` | 세 지점 통일. 새 테스트 3건, 고치기 전 코드에서 2건 실패 확인 |
| 2 | H2 | `a4ede3a` | 쓰기 거절 + serve SERVFAIL + 백스톱. 재현 테스트가 고치기 전 2초 타임아웃으로 실패 |
| 3 | M1 | `7569c80` | opcode NOTIMP, 클래스 REFUSED. 포워딩 투명성 테스트가 경계를 고정 |
| 3 | M2 | `369858d` | 노브 5종 노출, 기본값 불변 |
| 3 | M5 | `69a2b63` | 쿠키 리더 통합 + `__Host-`. 로그인 CSRF 와 복귀 경로 함정 둘 다 재현 후 고정 |
| 4 | M4 | `8b6ae53` | 데드라인. **리포트 서술 정정** — 아래 참조 |
| 4 | M3 | `a05b84f` | 괄호 이어짐. **결함 둘을 더 발견** — 아래 참조 |
| 5 | L1·L2·L3·L4·L8 | `fd4d3a3` | L1 은 서술 정정, L4 는 경고로 축소 |
| — | I1 절반 | `656a149` | 사라진 어댑터 주석 4곳 정리 |
| 6 | EDNS Cookie · BADVERS | `4aa24d4` | 결정이 필요 없던 둘 |
| 6 | L4 철회 · I1 NS1 삭제 | `5dc4d08` | 남긴 근거가 둘 다 틀렸다. 아래 참조 |
| — | 관측성 · SOA 값 설정 | `b0dbd91` | 계획 밖. §9 제안 중 결함에 가까웠던 둘 |

### 재현이 계획을 바꾼 곳 두 군데

**M4 — 원인의 절반이 틀렸다.** "유휴 keep-alive 커넥션이 종료를 붙잡는다"는 재현되지
않았다. Node 19+ 의 `server.close()` 는 유휴 커넥션을 스스로 닫는다. 남는 노출은
**응답하지 않는 in-flight 요청**이고 그것은 실재한다 — 멈춘 요청 하나가
`shutdownProcess` 를 120초 넘게 붙잡았고, 데드라인 뒤로는 310ms 다.
§0-4(고치기 전에 재현)가 없었다면 없는 문제를 고쳤을 것이다.

**M3 — 고치는 도중 같은 경로에서 결함 둘이 더 나왔다.** 괄호를 통과시키자
`uniqueId` 가 만드는 레코드 id 가 `validateRecordId` 의 두 규칙을 모두 어기는 것이
드러났다: underscored owner(`_dmarc`)가 `_` 로 시작하는 id 를 만들고, 길이를 60자로
자르는데 상한은 36자다. 둘 다 괄호와 무관하게 원래 있었고, 증상은 읽고 있는 파일과
무관해 보이는 "record id must be 1 to 36…" 이었다.

### L4 는 계획보다 좁게 넣었다

계획은 `ssl=true` 를 검증된 TLS 로 취급하지 않도록 조이라고 했다. 그러면 그 형태를
쓰는 기존 배포가 **기동에서 멈춘다**. 실제 `DATABASE_URL` 의 형태를 확인할 수 없으므로
거절이 아니라 경고를 넣었다. 조이는 것은 그 확인 뒤의 일이며, 그때는 한 줄이다.

### 6단계 — 넷은 했고, 셋은 결함이 아니다

**처음에 6단계 전부를 "사람이 정해야 하는 것" 으로 묶은 것은 뭉뚱그린 것이었다.**
다시 보니 그 안에 결정과 무관한 항목이 둘 섞여 있었고, 둘 다 했다(`4aa24d4`).

| 한 것 | 왜 결정이 아니었나 |
| --- | --- |
| **EDNS Cookie (RFC 7873)** | M2·L2의 실제 해법이고 하위 호환이다. 쿠키는 항상 주고받고 검증하되, 미증명 클라이언트를 어떻게 대할지는 `PARALLAX_DNS_REQUIRE_COOKIE` 로 열어두고 껐다. 쿠키를 보내지 않는 클라이언트는 이 커밋 전후가 같다 |
| **BADVERS** | 순전히 기술 작업이었다. M1에서 "확장 rcode 지원이 별건" 이라 미룬 것이지, 물어볼 것이 있어서가 아니었다. rcode 상위 8비트를 OPT의 TTL 첫 바이트에 쓴다 |

남겨둔 넷 중 **둘은 다시 확인해 보니 결정할 것이 없었다.** 남긴 근거가 둘 다 틀렸고,
확인하지 않고 판단한 것이 원인이다. 둘 다 처리했다(`5dc4d08`).

| 다시 본 것 | 남긴 근거 | 확인 결과 |
| --- | --- | --- |
| **L4** | "배포의 `DATABASE_URL` 을 알아야 조일 수 있다" | **발견 자체가 틀렸다.** `pg` 8.22.0 을 재보면 `ssl=true` 는 boolean `true` 가 되어 `tls.connect` 의 옵션 전체가 되고, Node 기본값 `rejectUnauthorized: true` 로 체인과 호스트명이 둘 다 검증된다. 거절되는 `sslmode=require` 쪽이 오히려 이 라이브러리에서 verify-full 의 별칭이다. 기존 코드가 맞았고, 조일 것이 없다. **`fd4d3a3` 이 이 틀린 판단으로 거짓 경고를 배포했던 것도 되돌렸다** |
| **I1 · NS1** | "남의 동작하는 코드를 지우는 결정" | **그 전제가 틀렸다.** NS1 은 감사 문서 바깥 어디에도 없다 — README 양쪽·`docs/`·`migrations/`·`package.json`·스크립트 전부. 광고된 적 없고 설정 경로가 없고 추가된 이래 도달 가능한 적이 없다. 지우는 것은 제품 변경이 아니라 정리고, 배선은 아무도 요청하지 않은 기능이라 범위 밖이다. 삭제했다 (`4cb844a` 에서 복구 가능) |

**남은 셋은 결함이 아니라 기능 제안이다.** 리포트 §9 "신규로 추가하면 좋을 기능" 에
있는 항목이고, §5 에 L5·L6 으로도 번호가 붙은 것은 "알려진 한계" 라는 뜻이지 고쳐야
할 버그라는 뜻이 아니다 — 두 항목의 분석이 모두 "설계 자체는 방어 가능하다" 로
끝난다. 요청 없이 만드는 것은 범위를 넓히는 일이고, 둘은 **살아 있는 배포의 인증
동작을 바꾼다.**

| 항목 | 성격 | 만들기로 한다면 물어야 할 것 |
| --- | --- | --- |
| **L5 · OIDC discovery** | 기능 (상호운용) | 지금 배포가 쓰는 프로바이더 하나만 상대하면 되는가. discovery 로 바꾸면 그 문서가 닿지 않는 환경에서 로그인이 죽는다 — 얻는 것은 이 배포에 없고 잃는 것은 이 배포에 있다 |
| **L6 · 세션 폐기** | 기능 (설계 변경) | 서버측 세션 인덱스는 상태를 늘린다(파일 백엔드에서 특히). `session-token.ts` 가 폐기 불가를 의도로 문서화하고 있으므로, 바꾸는 것은 그 의도를 뒤집는 일이다 |
| **존별 RBAC** | 기능 | 지금 editor 는 모든 존을 편집한다. 팀이 여럿인가 |

리포트 §9 의 나머지(TSIG, DoT/DoH, 드리프트 알림, `apply pending` dry-run,
`config check` 확장)도 같은 성격이며, 이 계획의 대상이 아니었다.

### 계획 밖에서 둘을 더 했다 — 2026-08-22, `b0dbd91`

§9 의 제안 중 둘은 다시 보니 "있으면 좋은 기능" 이 아니었다.

**관측성 (`/metrics`).** H2 가 살던 공백이 이것이다. 고쳐서 SERVFAIL 과 stderr
한 줄이 되었지만 **stderr 한 줄에는 임계값을 걸 수 없다** — 이번 수정이 넣은
`onUnservable`·`onUnanswerable` 신호도 지금까지 아무도 안 보는 곳으로만 나갔다.
세는 것의 규칙은 하나다: *그것이 유일한 경고인 실패만 센다.* 카운터는 일어나기
전에도 0 으로 존재하는데, 첫 실패 후에야 나타나는 시계열은 없음과 0 이 질의에서
구분되지 않아 그렇게 쓴 알림이 영원히 울리지 않기 때문이다. 존·레코드·클라이언트
이름은 레이블에 없고, `/health/ready` 가 상세를 두는 자리와 같은 인증 뒤에 있다.

로그 문장 23 개는 그대로 뒀다. 감사가 지적한 것은 "알림을 걸 표면이 없다" 이지
"문장이 읽기 어렵다" 가 아니고, 그 문장들은 사람이 읽으라고 쓴 것이다.

**SOA 값 설정.** 제안이라기보다 잠재 결함이었다. MNAME 은 세컨더리가 갱신을
물으러 가고 보내러 가는 곳인데 `ns.<zone>` 이 하드코딩되어 있었고, 그 이름은
존재가 확인된 적이 없다 — 모든 존에 apex 가 있다는 이유로 써넣은 것이다.
세컨더리가 없는 배포는 어느 쪽이든 모르고, 그래서 여태 남아 있었다. 이름은
기동에서 검증한다: 인코딩되지 않는 이름은 SOA 를 조립할 때마다 던지고, SOA 는
부정 응답이 담는 것이므로 증상이 "멀쩡해 보이는 리스너에서 NXDOMAIN 이 동작하지
않음" 이 되기 때문이다.

### 돌린 검증 — 2026-08-22, 커밋 `4aa24d4`

**처음에 `pnpm verify:*` 를 통째로 "실제 자원을 건드린다" 며 넘긴 것도 틀렸다.**
스크립트를 열어보니 넷 중 셋은 로컬이다.

| 스크립트 | 무엇을 쓰나 | 결과 |
| --- | --- | --- |
| `verify:dns` | 없음 — Docker 도 네트워크도 안 쓴다 | **통과.** 23개 타입이 실제 `dig` 로 파싱되고, 두 negative 구분, TC→TCP, 와일드카드, 전달까지. `dig` 는 기본으로 EDNS 쿠키를 보내므로 쿠키 왕복도 여기서 실제 클라이언트로 확인됐다 |
| `verify:proxy` | 일회용 로컬 nginx 컨테이너 + 자체 서명 인증서 | **통과.** H1이 바꾼 경로가 진짜 프록시 뒤 세션 쿠키로 확인됐다. 인증서 갱신 무중단 재적용까지 |
| `verify:postgres` | 일회용 로컬 postgres 컨테이너 | **통과.** 마이그레이션, advisory lock 하의 동시 apply, 재시작 지속성, 보존 정책, 존 삭제 캐스케이드 |
| `verify:cloudflare` | **실제 Cloudflare 계정** + `CF_VERIFY_ALLOW_WRITES=true` | **돌리지 않음.** 운영자가 쓰기를 허용할 존과 토큰을 직접 줘야 하는 opt-in 이다. 자격증명을 이 세션이 만들어 넣을 일이 아니다 |

⚠️ Cloudflare 어댑터 경로는 이 세션의 변경이 닿지 않았지만(`src/adapters/cloudflare.ts`
무변경), 그 한 건은 여전히 돌지 않았다. 배포 전에 돌린다면 **어느 커밋에서 돌렸는지
함께 적을 것** — 이 저장소는 과거에 스크립트가 깨진 채로 통과 기록만 남은 적이 있다.
