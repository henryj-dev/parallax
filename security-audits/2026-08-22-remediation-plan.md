...
>
> **이 문서는 계획으로 쓰였고, 아래 §7이 실행 기록이다.** 1~5단계는 끝났고,
> 6단계는 결정이 필요 없던 둘(EDNS Cookie, BADVERS)까지 했다. 남은 넷은
> 사람의 결정이 앞선다 — §7이 각각 무엇을 물어야 하는지 든다.
...
| 6 | EDNS Cookie · BADVERS | `4aa24d4` | 결정이 필요 없던 둘. 아래 참조 |
...
남은 넷은 실제로 결정이 앞선다. **그중 하나는 결정이 아니라 정보가 없어서 막혔다** —
구분해서 적는다.

| 항목 | 막힌 이유 | 물어야 할 것 |
| --- | --- | --- |
| **I1 · NS1** | 결정 | 배선인가 삭제인가. 삭제는 `git rm` 두 파일이고 이력(`4cb844a`)에 남는다. 배선은 자격증명 저장소를 Cloudflare 모양(profile = accountId + token) 밖으로 일반화하고 마이그레이션·포털·명령까지 따라가는 다일 작업이다. 남의 동작하는 코드를 지우는 쪽이든 없던 기능을 만드는 쪽이든, 이 세션이 혼자 정할 일이 아니다 |
| **L4 조이기** | **정보 없음** | 배포의 `DATABASE_URL` 이 `ssl=true` 형태인가. 아니면 한 줄이고, 맞으면 그 배포는 기동에서 멈춘다. 이 세션에서 확인할 방법이 없다 |
| **L5 · OIDC discovery** | 결정 | 지금 배포가 쓰는 프로바이더 하나만 상대하면 되는가, 상호운용이 요구사항인가. discovery 로 바꾸면 그 문서가 닿지 않는 환경에서 로그인이 죽는다 |
| **L6 · 세션 폐기** | 결정 | 서버측 세션 인덱스는 상태를 늘린다(파일 백엔드에서 특히). "전체 로그아웃 epoch" 라는 값싼 절충으로 충분한가 |
| **존별 RBAC** | 결정 | 지금 editor 는 모든 존을 편집한다. 팀이 여럿인가 |
