...
>
> **이 문서는 계획으로 쓰였고, 아래 §7이 실행 기록이다.** 1~5단계는 끝났다.
> 6단계는 사람의 결정이 앞서므로 손대지 않았다. 완료는 이 문서가 아니라 커밋과
> 테스트가 증명하며, §7이 항목마다 커밋 sha를 든다.
>
...
1~5단계 완료. 기준선 `95e85e6`(테스트 667) → `pnpm test` **699/699**, `pnpm check` 통과.
...
### 6단계 — 남은 것, 그리고 각각 무엇을 물어야 하는가

전부 코드가 아니라 결정이 앞선다. 손대지 않았다.

| 항목 | 물어야 할 것 |
| --- | --- |
| **I1 · NS1** | 배선인가 삭제인가. 지금은 테스트까지 딸린 224줄이 지원 기능처럼 보인다. 배선은 자격증명 저장소를 Cloudflare 모양 밖으로 일반화하는 큰 변경이고, 삭제는 그 코드를 버리는 결정이다 |
| **M2 · EDNS Cookie** | 노브 노출로 완화는 했다. RFC 7873 구현은 별건이고, 이 리스너가 실제로 신뢰 경계 밖에 노출되는지가 판단 근거다 |
| **L4 조이기** | 배포의 `DATABASE_URL` 이 `ssl=true` 형태인가. 아니면 한 줄로 끝난다 |
| **L5 · OIDC discovery** | 프로바이더 하나만 상대하면 되는가, 상호운용이 요구사항인가 |
| **L6 · 세션 폐기** | 서버측 세션 인덱스는 상태를 늘린다(파일 백엔드에서 특히). "전체 로그아웃 epoch" 라는 값싼 절충으로 충분한가 |
| **존별 RBAC** | 지금 editor 는 모든 존을 편집한다. 팀이 여럿인가 |
| **BADVERS** | `writeOpt` 의 TTL 상위 바이트를 쓰는 확장 rcode 지원. M1 에서 미룬 것 |

### 돌리지 않은 검증

`pnpm verify:*` 는 실제 자원을 건드리므로 이 세션에서 돌리지 않았다. 계획 §5가
1단계에 `verify:proxy`, 3단계에 `verify:dns`, 5단계 L4에 `verify:postgres` 를
요구한다 — **배포 전에 돌리고, 어느 커밋에서 돌렸는지 함께 적을 것.**
