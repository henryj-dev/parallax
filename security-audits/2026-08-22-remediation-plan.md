...
1~5단계 완료, 6단계는 결정이 필요 없던 둘까지. 기준선 `95e85e6`(테스트 667) →
`pnpm test` **706/706**, `pnpm check`·`pnpm build` 통과, `verify:dns`·`verify:proxy`·
`verify:postgres` 통과.
...
리포트 §9 의 나머지(SOA 값 설정, TSIG, DoT/DoH, 드리프트 알림, 관측성, `config check`
확장)도 같은 성격이며, 이 계획의 대상이 아니었다.
