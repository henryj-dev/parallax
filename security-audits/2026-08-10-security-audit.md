...
>
> 📝 **2026-08-23 리댁션.** 저장소 퍼블릭 전환에 맞춰 §9.8 과 위 표의 실계정 검증에서
> 존 이름만 지웠습니다. 실계정에서 돌렸다는 사실도, 무엇이 나왔는지도 그대로입니다 —
> 지운 것은 그 계정이 누구 것인지뿐입니다.
...
| Cloudflare | `pnpm verify:cloudflare` | **통과 (`ef61201`, 2026-08-12, 실계정(존 이름은 비공개))** — 라이브 존 read, 소유하지 않은 3건 앞에서 삭제 제안 0건, 발행, 드리프트 0(자기 쓰기 왕복), proxied TTL Auto 정규화, 존 삭제 시 회수. 자격 증명이 없으면 건너뛴다. **이 실행이 결함 셋을 찾았다 — 9.8 참조** |
...
토큰을 쥔 운영자가 실계정(존 이름은 비공개)에서 실행했다. **네 번 돌렸고 결함 셋이 나왔다.**
