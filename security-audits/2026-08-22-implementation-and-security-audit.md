...
> 읽기 전용 감사. 이 보고서 파일 외에 코드·설정은 수정하지 않았다.
> 실제 `.env`는 열지 않았고 `pnpm verify:*` 등 실 자원 변경 스크립트는 실행하지 않았다.
> 동적 확인은 전부 폐기 가능한 스크래치패드 스크립트(인메모리 저장소·루프백 포트)로만 수행했다.
>
...
- **영향:** `shutdownProcess`가 `server.close()`를 await 한다. 이는 열린 커넥션이 전부
  끝나야 콜백이 온다. keep-alive 유휴 커넥션은 `keepAliveTimeout`(10s), 진행 중 요청은
  `requestTimeout`(60s)까지 버틴다. `closeIdleConnections()` 호출도, 강제 종료 타이머도
  없다. 최악의 경우 종료가 60초를 넘겨 Kubernetes가 SIGKILL을 보내고, **그 시점이 apply
  도중이면** 프로바이더에는 일부만 적용된 상태가 남는다.
- **확신도:** 코드 추적으로 확인.
...
- **확신도:** 코드 추적으로 확인. 브라우저 실제 재현은 미수행.
...
### L4. `usesPlaintextPostgres`의 판정이 비일관적이다
- **위치:** `src/config.ts:350-353`
- **분석:** `?ssl=true`는 "평문 아님"으로 통과시키면서 `sslmode=require`는 평문 취급해
  거부한다. `ssl=true`가 `require`보다 강한 호스트명/CA 검증을 보장하지 않으므로,
  더 엄격해 보이는 검사에 방향이 반대인 구멍이 있다.
- **수정 제안:** `ssl=true`도 `verify-ca`/`verify-full`과 동급으로 취급하지 않는다.
...
- **수정 제안:** ①NS1을 배선하거나(자격증명 저장소를 Cloudflare 모양 밖으로 일반화)
  **삭제한다** — 지금은 테스트까지 딸린 죽은 코드가 지원 기능처럼 읽힌다.
  ②사라진 어댑터를 가리키는 주석 네 곳을 정리한다.
...
보안 보고 쪽), 기능 결함 5건(H1, H2, M1, M3, I1), 신뢰성 3건(M4, L4, L8), 잔여 위험 1건(L7).
