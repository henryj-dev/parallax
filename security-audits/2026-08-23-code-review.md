...
> 🔴 **그 뒤 그 수정들 자체가 검수를 받았고, 통과하지 못했다** — 소유권 위조를
> 포함해 약 40건. [§13](#13-새-코드에-대한-검수--2026-08-24) 을 먼저 볼 것.
>
> 📌 **2026-08-24: 20건 전부 해소됐다.** 무엇이 어느 커밋으로, 그리고 발견과 다르게
> 고친 곳은 [§12](#12-그-뒤--2026-08-24-해소-현황) 에 덧붙였다. 아래 §0–§11 은
> 2026-08-23 스냅샷 그대로이고 손대지 않는다 — 지금 고치면 그때 무엇이 보였는지가
> 사라진다. §2 의 「재현」 열은 *재현했다* 는 뜻이지 *고쳤다* 는 뜻이 아니다.
...
## 12. 그 뒤 — 2026-08-24 해소 현황

⚠️ **이 절은 덧붙인 것이고, 위의 §2–§11 은 손대지 않았다.** 저것은 2026-08-23 에
무엇이 보였는지의 기록이라, 지금 와서 고치면 그때 보였던 것이 사라진다. §2 의 「재현」
열도 그대로다 — 그 열은 *재현했다* 는 뜻이지 *고쳤다* 는 뜻이 아니었고, 뜻을 바꿔
쓰면 표가 두 가지를 말하게 된다.

살아 있는 진행 목록은 [`docs/todo.md`](../docs/todo.md) 에 있다. 무엇을 어떻게
고쳤는지·계획이 틀렸던 곳은
[계획 문서 §13](2026-08-23-remediation-plan.md#13-실행-기록) 에 있다.

**20건 전부 해소.** 여기서 제안만 하고 사람에게 넘겼던 §9 중 여섯도 함께 들어갔다.

| # | 커밋 | 실제로 한 일 (발견과 다른 경우만) |
| --- | --- | --- |
| H1 | `d5900a4` | |
| H2 | `6e72bef` · `4f5e667` | 두 커밋. 먼저 실패를 들리게 하고(T4), 그다음 AXFR 을 여러 메시지로 나눴다(T18) |
| H3 | `ca23ad3` | |
| M1 | `5dab45c` | **계획대로 하지 않았다.** 「없으면 기동을 막는다」였는데, 마스터키만 있고 바인딩이 없는 조합은 정상 기동이고 문서가 안내하는 순서다. 변수 이름·경고·`config check` 만 고치고, 막지 않는다는 결정을 검사로 박았다 |
| M2 | `6953b9c` | |
| M3 | `2af89b6` | 이 수정의 검사가 **기존 결함 하나를 더 드러냈다** — 알림이 복수형 처리 없이 `translate()` 로 바로 가서 존 삭제 알림이 키 문자열을 그대로 찍고 있었다 |
| M4 | `e67f97e` | `/proc/<pid>/stat` 22번 필드로 pid 재사용을 구분한다. macOS 에는 없어 그쪽은 종전대로 |
| M5 | `0a83eb3` | **계획대로 하지 않았다.** 「존 락을 상태 파일 락에서 분리」였는데 이미 다른 파일이었다. 대신 타임아웃 120초 + 「살아 있는 보유자」를 구분하는 메시지 |
| M6 | `f87901e` | 429 는 모든 메서드, 5xx·전송 실패는 멱등 메서드만 |
| M7 | `6be6d23` | |
| L1 | `ba7a01c` | |
| L2 | `26a908a` | |
| L3 | `95b160a` | **「성능」 분류가 좁았다.** 쓰기만이 아니라 읽기도 물려 있었다 — §5 는 `#mutate` 를 가리켰는데 `list()` 가 같은 값을 치르고 있었다. 아래 표 |
| L4 | `1f8c6d8` | |
| L5 | `b19248b` | |
| L6 | `033974e` (라벨 안의 점) · `a7ad449` (CNAME 체인) | |
| I1 | `11e758a` | 다섯 곳이라고 적었는데 **컴파일러가 열한 곳을 찾았다.** `noUnusedLocals`·`noUnusedParameters` 를 켜서 다음부터는 사람이 세지 않는다 |
| I2 | `07873a1` | |
| I3 | `48502d5` | |
| I4 | `5fadf36` | 문서 한 줄로 닫는 쪽이 아니라 **discovery 구현**을 골랐다 |

### §9 의 제안 중 들어간 것

| | 커밋 | |
| --- | --- | --- |
| D1 · OIDC discovery | `5fadf36` | 위 I4 |
| D2 · 토큰 만료·마지막 사용 | `3d16e3e` | 마이그레이션 `005` |
| D3 · 백업·복원 | `b44d9c7` | **백업 경로가 곧 파일→PostgreSQL 이관 경로다.** 두 백엔드가 같은 포트를 구현하므로 따로 만들 것이 없었다 |
| D5 · `zone import` 본문 상한 | `435cd39` | 경로별 상한. 스트리밍은 여기서 벌 것이 없다 |
| D6 · 요청 로그·DNS 메트릭 | `a46a33c` | 요청 ID · 접근 로그 한 줄 · 히스토그램 |
| D7 · AXFR·NOTIFY TSIG | `dee969e` | 이 리포트와 이전 리포트가 둘 다 제안했던 것 |
| T18-b · IXFR | `b0f20ac` | 계획은 변경 저널이 필요하다고 했으나 **필요 없었다** — serial 이 곧 revision 이라 스냅샷 둘을 비교하면 된다 |
| **D4 · 두 번째 프로바이더** | — | **아직 물음.** `router.ts` 의 `internal` 생성자 옵션은 I1 정리에서 일부러 남겼다 |

### L3 을 재보니 읽기도 물려 있었다

같은 맥에서 **연달아 세 쌍**을 잰 A/B — 10존 × 200레코드 × 리비전 50(기본 리텐션의
절반). 아래는 세 회차의 중앙값이다:

| | v1 | v2 | 배수 |
| --- | --- | --- | --- |
| 상태 파일 | 19.9 MiB | **0.4 MiB** | 50× |
| 레코드 하나 변경 | 353 ms | **64 ms** | 5.5× |
| `list()` | 253 ms | **7.6 ms** | 33× |
| 500 커밋 구축 | 126 s | **26.5 s** | 4.8× |

⚠️ **처음에는 1778 ms → 124 ms 라고 적었고, 그것은 틀렸다.** v1 쪽이 실행마다
흔들리는데 그중 나쁜 회차를 집은 것이다. 세 회차의 v1 은 341 · 389 · 353 ms,
v2 는 67 · 64 · 61 ms 였다. 커밋 `95b160a` 의 메시지에는 옛 숫자가 남아 있다 —
푸시된 이력이라 고치지 않는다. **쓰기 5–6배, 읽기 30배**가 맞는 값이다.

`list()` 가 저기 있는 것이 §5 가 놓친 부분이다. 「존이 뭐가 있나」를 묻는데 모든
리비전의 모든 레코드를 파싱하고 `createDesiredRecord` 로 재검증하고 있었다.

### 검사

**770 → 879.** `check`(Node 24·26) · `scripts` · `docker` · `codeql` 네 워크플로가
서른한 커밋 전부에서 초록이다. D7 과 T18-b 는 BIND 9.10.6 의 `dig` 로 따로 쟀고,
두 번 다 **대조군**을 함께 뒀다 — 계획 문서 §13.

### 이 검수가 만든 결함 둘

작성 중 검사가 잡았고 둘 다 D7 에서 나왔다: `10.0.0.2#` 이 주소에 `#` 를 남긴 것과,
`verifyTsig` 가 응답을 검증할 방법이 없어 요청 MAC 접두를 빼먹은 것. 뒤엣것은 `dig`
가 통과시킨 것을 내 검사가 거절해서 드러났다.

---

## 13. 새 코드에 대한 검수 — 2026-08-24

§12 가 「20건 전부 해소」로 끝난 그 코드에, **아무도 검수하지 않은 4,040줄**이
들어 있었다. 같은 사람이 쓰고 같은 사람이 검사를 붙였으니 통과는 증거가 아니다.
`CLAUDE.md` 가 정한 대로 **별도 레인** 셋에 나눠 돌렸다 — 보안, 와이어 파싱,
저장·동시성.

**세 레인 전부 REQUEST CHANGES.** 약 40건이 나왔고, 무거운 다섯을 먼저 직접
재현했다. **다섯 다 진짜였다.**

| | 무엇이 | 재현 |
| --- | --- | --- |
| 🔴 | **RFC 2136 소유권 마커가 위조된다** | 익스플로잇 성공: 남의 레코드가 우리 키로 삭제됐다 |
| 🔴 | 응답 TSIG 를 아무도 검증하지 않는다 | `verifyTsig` 호출부가 서버 인바운드 한 곳뿐 |
| 🔴 | 2바이트가 프로세스를 죽인다 | `UNCAUGHT: RangeError`, 소켓 리스너 안 |
| 🔴 | 찢어진 감사 줄 하나가 저장소를 잠근다 | `audit()` 과 이후 모든 쓰기가 던짐 |
| 🔴 | 복원이 대상의 자격증명을 파괴한다 | 대상 암호문이 원본 것으로 교체됨 |

### 위조가 어떻게 됐나

`ownershipComment` 는 `(target, recordId)` 만 서명한다. 마커가 주장하는 타입과 내용
다이제스트는 **서명 밖 평문**이었고, 어느 레코드를 가리키는지는 **TXT 가 앉은
자리**였다. 셋 다 공격자가 고른다.

```
공개 마커 (dig 로 누구나):  A 440a628a… parallax-managed:v3:blog:AwSF8k_3…
_parallax.www 에 심은 위조: A 2ba3fdf2… parallax-managed:v3:blog:AwSF8k_3…  ← 서명 그대로
list(): managed=true id=blog  →  재조정: delete:www  →  남의 레코드 사라짐
```

존에 TXT 하나 쓸 권한이면 된다. `apply` 의 가드 둘 다 통과했다 — 둘 다 「마커가
검증되나」를 물었지 「**이 레코드에 대해** 검증되나」를 묻지 않았다.

### 왜 996개 검사가 이걸 다 통과시켰나

리뷰어 하나가 근본 원인을 정확히 짚었다. `rdata-decode.ts` 의 확인된 결함이 **전부**
「바이트가 이 인코더에서 왔다」를 가정한 자리이고, 왕복 검사(`encode → decode →
encode`)는 **바로 그 가정을 강제**한다. 샘플이 전부 인코더에서 나오니 여덟 개가
전부 통과했다.

📌 **검사가 통과하는 것과 검수를 받은 것은 다르다.** 그 차이가 이 절의 전부다.
...
| --- | --- |
| `b4973fa` | 위 다섯 |
| `e0064a5` | 디코더 여덟 — TXT 조각 병합 · 비-UTF8 · CAA 태그 · alpn 쉼표 · IPv4 매핑 · 빈 꼬리 · LOC 범위 · alias 폼 |
| `017a027` | IXFR serial 산술(RFC 1982) · CNAME 체인 끝 rcode · TSIG 키 이름 로그 주입 · 안 만료되는 토큰 |
| `030ed97` | OIDC discovery issuer 미검증 · in-memory 더블 · append 디렉터리 fsync · export 스냅샷 · 복원 사전검증 |
| `a0b645d` | 반쪽 RRset · SVCB mandatory 규칙 · 이름 안의 공백 |
| `77d0841` | 응답 대조 · 확장 rcode · UDP 예산 · 백프레셔 · 임시파일 · 읽기 권한 · SOA 소유 이름 |

**검사 996 → 1,022.** 새 검사는 전부 **변이로 물리는지 확인**했다 — 스무 개 넘게
돌렸고 전부 잡힌다. 특히 디코더 쪽은 **적대적 바이트에서 시작하는** 검사를 새로
만들었다. 왕복 검사가 구조적으로 못 잡는 자리라서다.

### 이번에 내가 만든 결함 둘

- 「거절당한 전송이 타임아웃 난다」를 고치며 넣은 줄이 **2바이트 프로세스 킬**이었다.
  검사가 잘 행동하는 가짜 서버만 썼으니 잡힐 리가 없었다
- 임시파일 청소를 pid 로 거르려다, **동시에 쓰는 다른 프로세스의 파일을 지우는**
  것을 알아채고 되돌렸다. 새는 것보다 나쁘다

### 그리고 §11 이 「돌리지 않았다」고 적은 것 중 셋

| | |
| --- | --- |
| `verify:postgres` | 실제 PostgreSQL 17 상대 14개 검사 통과 |
| `verify:dns` | 실제 `dig` 상대 18개 검사 통과, 수정 후 재실행도 통과 |
| `verify:proxy` | 실제 nginx + TLS 상대 17개 검사 통과 |
| `verify:cloudflare` | **여전히 안 돌림** — 실제 자격증명이 필요하다 |
| 브라우저 포털 실조작 | **여전히 안 함** |
