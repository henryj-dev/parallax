...
CI에서 세 워크플로가 돕니다. `check`는 커밋에 귀속되는 판정들을 모으고, 각 잡이 다른
질문에 답하므로 빨간 결과가 스스로 원인을 말합니다: `verify`(타입·빌드·테스트, Node 24와
...
`flake-watch`(스위트를 세 번, **주기 실행에서만** — 간헐 실패는 코드가 그대로여도 판정이
바뀌는 종류입니다). `gate` 하나가
그것들을 모으며, 브랜치 규칙이 요구하는 체크 이름은 그것뿐입니다 — 잡 이름을 하나씩
나열하면 이름이 바뀌는 날 그 필수 체크는 존재하지 않는 체크가 되고 보호가 조용히
사라집니다. `codeql`과 `scorecard`는 각자의 일정으로 돕니다. 코드가 그대로여도 판정이
바뀌기 때문입니다.
