...
describe("managed-only reconciliation", () => {
  it("creates missing records and updates managed records", () => {
    ...
    assert.deepEqual(buildReconcilePlan([desired], [actual]).summary, { create: 0, update: 1, delete: 0, conflict: 0 });
  });
  ...
  it("deletes only managed records and reports unmanaged collisions", () => {
    ...
    assert.deepEqual(buildReconcilePlan([], [obsolete, foreign]).summary, { create: 0, update: 0, delete: 1, conflict: 0 });
  ...
  it("reconciles every value in a multi-value RRset without deleting sibling values", () => {
    ...
    assert.deepEqual(plan.summary, { create: 0, update: 1, delete: 0, conflict: 0 });
  ...
  it("matches duplicate provider values one-to-one and deletes only the surplus managed copy", () => {
    ...
    assert.deepEqual(plan.summary, { create: 0, update: 0, delete: 1, conflict: 0 });
  ...
  it("does not drift when Cloudflare reports Auto TTL for a proxied record", () => {
    ...
    assert.deepEqual(buildReconcilePlan([proxied], [actual]).summary, { create: 0, update: 0, delete: 0, conflict: 0 });
  });
});
