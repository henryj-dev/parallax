...
마지막 갱신: **2026-08-31** (Asia/Seoul) — 「현재 상태」만. 아래 「지나온 것」은 그때그때의
기록이고 고치지 않는다. 이전 갱신은 2026-08-23 · `1303d46`.
...
```
테스트          1096 중 1093 통과 · 3 스킵 · tsc 통과 · build 통과 · 포털 타입 검사 통과
CI              check(verify · deployment-gate · docker · hooks · shellcheck · policy ·
                flake-watch → gate) · codeql · scorecard — 워크플로 파일은 셋
실제 의존성      PostgreSQL 17 · nginx TLS 종단 · Cloudflare 실계정
배포            컨테이너 이미지, read-only 루트, uid 10001, initContainer 로 스키마 적용
```

⚠️ **첫 줄과 둘째 줄은 2026-08-31 에 측정한 것이고, 아래 「지나온 것」은 그때그때의 기록이라
고치지 않는다.** 그 구분이 이 문서에서 한 번 무너졌다: 「테스트 770/770 · CI 다섯
워크플로」가 `1303d46` 시점의 값으로 남아 있었고, 그 사이 테스트는 1096 개가 되고 워크플로는
셋이 됐다. **핸드오프는 현재를 말하는 것이 목적인 문서**라 날짜가 박혀 있어도 스냅샷처럼
둘 수 없다 — 아래 감사 보고서·계획과 다른 점이 그것이다.

📌 셈이 정확히 맞지 않는 날이 있다. `docs/test-flakes.md` 에 2026-08-31 관찰 두 건이 있고,
둘 다 정확히 한 건이 통과도 스킵도 아닌 상태로 끝났으며 재현되지 않았다. 스위트가 빨개졌는데
다음 실행에서 통과하면 **그 파일을 먼저 볼 것.**

⚠️ **셋째 줄만은 다시 재지 못했다.** 실제 의존성 대상 검증은
