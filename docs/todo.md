...
- [ ] **T6** · `S` · ownership secret 누락을 `readConfig` 에서 이름 대고 거절 — `src/config.ts`
      <br>🔴 **기동을 막는 변경.** 실제 배포의 환경변수 조합을 먼저 확인

## 3단계 — 정확성 (테스트를 먼저 쓴다)

- [ ] **T7** · `S` · 와일드카드를 closest encloser 에서 멈추게 — `src/dns/server.ts:660`
      <br>🔴 **지금 답하던 이름이 NXDOMAIN 이 된다.** 옳은 변경이지만 체감은 장애 —
      배포 전 영향받는 이름을 desired state 에서 뽑을 것
